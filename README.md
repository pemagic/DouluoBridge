...
> **Cross-Platform Edition** v1.8.15 — 中国水墨风武侠横版动作游戏 (iOS & Android)
...
### 💥 v1.8.15 十项底层重构与体验跨越
1. **绝对防御沉浸系统**：Android 引擎已注入 `LAYOUT_IN_DISPLAY_CUTOUT_MODE_ALWAYS` 与 `max_aspect` 破界参数，完美刺穿任何机型的刘海与水滴黑边。
2. **极速零耗 GC 渲染**：彻底替换 Android 版本绘制系统！抛弃数千次位图新建，所有跳跃台与渐变长空全凭单像素 $1\times 1$ 全局色块复用矩阵拉伸达成，永久根除“游戏切落与进关重开”的暴雪级掉帧。
3. **高保真拟物美术重做**：废弃廉价色块。重新设计出 `45px` 点阵精绘尺寸心型血包、金色护手长剑与六芒星法术书道具。
4. **炫酷粒子打击感**：手搓出高阶 LibGDX Action 库模拟粒子库。每次击杀都会炸裂同色的碎块飞扬，点击无敌冲锋(Dash)时身后更有赛博幻影视界尾迹！
5. **UI 排长延迟根除**：双端气泡悬浮提示弹跳逻辑优化，动画中断队列重铸后道具反馈永不少漏！
6. **逻辑反跳关防御体系**：扫除隐藏在底层的物理死循环调用，终结一枪爆出双倍神装且莫名跨级跳关的致命 Bug。
7. **Kamikaze 防误触架构**：拦截具有近身自毁特性的 Chaser 类型变异当选 Boss 后的秒杀降维判定。
8. **精准动态视距校对**：通过调节 `PlayerNode` 和 `EnemyNode` 极坐标锚点系数提升 30% 模型可视规格。
9. **实心高保真着色系统**：全系列抹除 Android 色标换算中的隐性透明缩放通道误伤矩阵，原汁原味还原百分之百死实心高饱和不透明像素体格感。
10. **智能海浪步面**：抛弃下缘直边绘制结构。现已全局植入计算量极其微小但视觉超灵动的正弦波边缘算法波动地坪。
